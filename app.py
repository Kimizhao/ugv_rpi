...
app = Flask(__name__)
app.secret_key = "ugv_rpi_secret_key"
...
def login():
    ...
    if request.method == "POST":
        ...
        # Get credentials from config, default to admin/password
        login_conf = f.get("login_config", {})
        conf_user = login_conf.get("username", "admin")
        conf_pass = str(login_conf.get("password", "password"))
...
def get_video_names():
    video_files = sorted(
        [
            filename
            for filename in os.listdir(thisPath + "/templates/videos/")
            if filename.endswith(".mp4")
        ],
        key=lambda filename: os.path.getctime(
            os.path.join(thisPath + "/templates/videos/", filename)
        ),
        reverse=True,
    )
...
def get_audio_files():
    files = [
        ...
        and (f.endswith(".mp3") or f.endswith(".wav"))
    ]
...
def update_data_websocket_single():
    ...
    try:
        socket_data = {
            ...
            f["fb"]["base_voltage"]: base.base_data["v"] if base.base_data else 0,
...
def update_data_loop():
    ...
    while 1:
        ...
        base.base_oled(
            3,
            f"{si.wifi_mode} {hours:02d}:{minutes:02d}:{seconds:02d} {si.wifi_rssi}dBm",
        )
        time.sleep(5)
...
def cmd_on_boot():
    cmd_list = [
        ...
        'base -c {{"T":4,"cmd":{}}}'.format(
            f["base_config"]["module_type"]
        ),  # select the module - 0:None 1:RoArm-M2-S 2:Gimbal
